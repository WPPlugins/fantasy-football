 ...
 * Plugin URI: https://www.fantasyfootballnerd.com/wordpress
 ...
 * Version: 1.4.0
 * Author: TayTech, LLC
 * Author URI: https://www.fantasyfootballnerd.com/wordpress
...
add_shortcode('ffn_draft_idp', 'ffnDraftIDP');
...
add_shortcode('ffn_daily', 'ffnDaily');
add_shortcode('ffn_dynasty', 'ffnDynasty');
add_shortcode('ffn_news', 'ffnNews');
...
/**
 * Display the IDP draft rankings from FantasyFootballNerd.com
 *
 * No params are expected
**/
function ffnDraftIDP(){
	$output = array();
	//-- Get the IDP rankings --
	$data = ffnCallAPI('draft-idp', 'xml', array());
	$doc = new DOMDocument(); 
	$doc->loadXML($data);
	if ($doc->getElementsByTagName("Error")->length > 0){
		$output[] = "<p>Error: " . $doc->getElementsByTagName("Error")->item(0)->nodeValue . "</p>";
		...
		$output[] = "<div class='ffntable-responsive'><table class='ffntable ffntable-striped'><thead>";
		$output[] = "<tr><th>Rank</th><th>Player</th><th>Team</th><th>Position</th><th>Bye</th></tr>";
		$output[] = "</thead><tbody>";
		foreach ($doc->getElementsByTagName("Player") AS $player){
			$output[] = "<tr><td>" . $player->getElementsByTagName('rank')->item(0)->nodeValue . "</td><td>" . $player->getElementsByTagName('player')->item(0)->nodeValue . "</td><td>" . $player->getElementsByTagName('team')->item(0)->nodeValue . "</td><td>" . $player->getElementsByTagName('position')->item(0)->nodeValue . "</td><td>" . $player->getElementsByTagName('bye')->item(0)->nodeValue . "</td></tr>";
		}
		$output[] = "</tbody></table></div>";
	}
...
/**
 * Display the DFS rankings from FantasyFootballNerd.com
 *
 * @param array $params An array of values provided in the shortcode. Expecting 'platform' => 'fanduel', 'week' => '1'. Options: fanduel, draftkings, yahoo. Default: fanduel. If no week specified, it will default to current week.
**/
function ffnDaily($params){
	$output = array();
	$players = array();
	$bang = array();
	if (isset($params['platform']) && in_array(strtolower($params['platform']), array('fanduel','draftkings','yahoo'))){$platform = strtolower($params['platform']);}else{$platform = 'fanduel';}
	
	//-- Get the Daily rankings --
	$data = ffnCallAPI('daily', 'xml', array('platform' => $platform));
	$doc = new DOMDocument(); 
	$doc->loadXML($data);
	if ($doc->getElementsByTagName("Error")->length > 0){
		$output[] = "<p>Error: " . $doc->getElementsByTagName("Error")->item(0)->nodeValue . "</p>";
	}else{
		//-- Order by the consensus projected points. Perhaps someday make this configurable --
		foreach ($doc->getElementsByTagName("Player") AS $player){
			$playerId = $player->getElementsByTagName('PlayerId')->item(0)->nodeValue;
			if (!isset($bang[$playerId])){$bang[$playerId] = 9999;}
			$players[$playerId] = array("name" => $player->getElementsByTagName('Name')->item(0)->nodeValue, "team" => $player->getElementsByTagName('Team')->item(0)->nodeValue, "position" => $player->getElementsByTagName('Position')->item(0)->nodeValue, "salary" => $player->getElementsByTagName('Salary')->item(0)->nodeValue);
			foreach ($player->getElementsByTagName('Projections')->item(0)->getElementsByTagName("Option") AS $proj){
				if ($proj->getAttribute("Type") == 'consensus'){$bang[$playerId] = $proj->getElementsByTagName("BangForYourBuckScore")->item(0)->nodeValue;}
			}
		}
		asort($bang);//sort the bang scores but keep the key association
		$output[] = "<div class='ffntable-responsive'><table class='ffntable ffntable-striped'><thead>";
		$output[] = "<tr><th>Player</th><th>Team</th><th>Position</th><th>Salary</th></tr>";
		$output[] = "</thead><tbody>";
		foreach ($bang AS $playerId => $score){
			if ($score > 0){$output[] = "<tr><td>" . $players[$playerId]['name'] . "</td><td>" . $players[$playerId]['team'] . "</td><td>" . $players[$playerId]['position'] . "</td><td>\$" . number_format($players[$playerId]['salary']) . "</td></tr>";}
		}
		$output[] = "</tbody></table></div>";
	}
...
function ffnInjuries($params){
	...
	if (isset($params['team'])){$tm = $params['team']; unset($params['team']); }else{$tm = '';}
	if (!isset($week)){
		$data = ffnCallAPI('schedule', 'xml');
		$doc = new DOMDocument(); 
		$doc->loadXML($data);
	...
	if ($count < 1){$output[] = "<p>There are no injuries to display.</p>";}
...
function ffnGamedayInactives($params){
	...
	$output = array();
	if (isset($params['week']) && $params['week'] > 0 && $params['week'] <= 17){$week = $params['week'];}else{$week = 0;}
	if (isset($params['team'])){$team = $params['team'];}else{$team = '';}
	if ($week < 1){
		$data = ffnCallAPI('schedule', 'xml');
		$doc = new DOMDocument(); 
		$doc->loadXML($data);
	...
	$p = array('week' => $week);
	$data = ffnCallAPI('inactives', 'xml', $p);
	$doc = new DOMDocument(); 
	$doc->loadXML($data);
	if ($doc->getElementsByTagName("Error")->length > 0){
		$output[] = "<p>Error: " . $doc->getElementsByTagName("Error")->item(0)->nodeValue . "</p>";
		...
		foreach ($doc->getElementsByTagName("Player") AS $player){
			//-- Are we asking for a specific team? --
			if (($player->getAttribute('team') == $team) || $team == ''){
				$count++;
	...
	if ($count < 1){$output[] = "<p>There are no players to display.</p>";}
...
/**
 * Display dynasty rankings from FantasyFootballNerd.com
 *
 * No params are expected
**/
function ffnDynasty(){
	$output = array();
	//-- Get the Dynasty rankings --
	$data = ffnCallAPI('dynasty', 'xml', array());
	$doc = new DOMDocument(); 
	$doc->loadXML($data);
	if ($doc->getElementsByTagName("Error")->length > 0){
		$output[] = "<p>Error: " . $doc->getElementsByTagName("Error")->item(0)->nodeValue . "</p>";
...
/**
 * Display the latest news from FantasyFootballNerd.com
 *
 * @param array $params An array of values provided in the shortcode. Expecting 'type' => 'nfl' if requesting NFL news or 'type' => 'players' if requesting Player news, 'max' => '10' for max stories per topic
**/
function ffnNews($params){
	$output = array();
	$type = '';
	$max = 10;
	if (isset($params['type']) && strtolower($params['type']) == 'nfl'){$type = 'nfl';}
	if (isset($params['type']) && strtolower($params['type']) == 'players'){$type = 'players';}
	if (isset($params['max'])){$max = (int)$params['max'];}
	//-- Get the news --
	$data = ffnCallAPI('news', 'xml', array());
	$doc = new DOMDocument(); 
	$doc->loadXML($data);
	if ($doc->getElementsByTagName("Error")->length > 0){
		$output[] = "<p>Error: " . $doc->getElementsByTagName("Error")->item(0)->nodeValue . "</p>";
		...
		if ($type == '' || $type == 'nfl'){
			$output[] = "<h4 class='ffn_h4'>NFL News</h4>";
			$articleCount = 1;
			foreach ($doc->getElementsByTagName("NFL")->item(0)->getElementsByTagName('Article') AS $article){
				if ($articleCount <= $max){
					$output[] = "<p><strong><a href=\"" . $article->getElementsByTagName('StoryLink')->item(0)->nodeValue . "\" target=\"_blank\" rel=\"nofollow\">" . $article->getElementsByTagName('Headline')->item(0)->nodeValue . "</a></strong><br>";
					$output[] = $article->getElementsByTagName('Story')->item(0)->nodeValue . "<br><small>(" . date("M j", strtotime($article->getElementsByTagName('StoryDate')->item(0)->nodeValue)) . " -- " . $article->getElementsByTagName('SourceName')->item(0)->nodeValue . ")</small></p>";
					$articleCount++;
				}
			}
		}
		if ($type == '' || $type == 'players'){
			$output[] = "<h4 class='ffn_h4'>Player News</h4>";
			$articleCount = 1;
			foreach ($doc->getElementsByTagName("Players")->item(0)->getElementsByTagName('Article') AS $article){
				if ($articleCount <= $max){
					$output[] = "<p><strong><a href=\"" . $article->getElementsByTagName('StoryLink')->item(0)->nodeValue . "\" target=\"_blank\" rel=\"nofollow\">" . $article->getElementsByTagName('Headline')->item(0)->nodeValue . "</a></strong><br>";
					$output[] = $article->getElementsByTagName('Story')->item(0)->nodeValue . "<br><small>(" . date("M j", strtotime($article->getElementsByTagName('StoryDate')->item(0)->nodeValue)) . ")</small></p>";
					$articleCount++;
				}
			}
		}
...
function ffnCallAPI($service, $format, $options = array()){
	//-- Build the URL --
	$apiKey = ffnGetKey();
	$url = 'https://www.fantasyfootballnerd.com/service/' . $service . '/' . $format . '/' . $apiKey;
	...
	if (strtolower($apiKey) == 'test'){echo "<p>** TEST DATA **</p><p>To retrieve live data, please create a <a href='https://www.fantasyfootballnerd.com/fantasy-football-api' target='_blank'>free API key</a>. Instructions for working with the FFN WordPress plugin can be found <a href='https://www.fantasyfootballnerd.com/wordpress' target='_blank'>here</a>.</p>";}
	return $body;
}
...
function ffnGetTeamName($team, $fullname = false){
	$teamNames = array("Arizona", "Atlanta", "Baltimore", "Buffalo", "Carolina", "Chicago", "Cincinnati", "Cleveland", "Dallas", "Denver", "Detroit", "Green Bay", "Houston", "Indianapolis", "Jacksonville", "Kansas City", "Miami", "Minnesota", "N.Y. Giants", "N.Y. Jets", "New England", "New Orleans", "Los Angeles", "Philadelphia", "Pittsburgh", "Los Angeles", "San Francisco", "Seattle", "St. Louis", "Tampa Bay", "Tennessee", "Washington");
	$fullTeamNames = array("Arizona Cardinals", "Atlanta Falcons", "Baltimore Ravens", "Buffalo Bills", "Carolina Panthers", "Chicago Bears", "Cincinnati Bengals", "Cleveland Browns", "Dallas Cowboys", "Denver Broncos", "Detroit Lions", "Green Bay Packers", "Houston Texans", "Indianapolis Colts", "Jacksonville Jaguars", "Kansas City Chiefs", "Miami Dolphins", "Minnesota Vikings", "N.Y. Giants", "N.Y. Jets", "New England Patriots", "New Orleans Saints", "Los Angeles Raiders", "Philadelphia Eagles", "Pittsburgh Steelers", "Los Angeles Chargers", "San Francisco 49ers", "Seattle Seahawks", "St. Louis Rams", "Tampa Bay Buccaneers", "Tennessee Titans", "Washington Redskins");
	$myNames = array("ARI", "ATL", "BAL", "BUF", "CAR", "CHI", "CIN", "CLE", "DAL", "DEN", "DET", "GB", "HOU", "IND", "JAC", "KC", "MIA", "MIN", "NYG", "NYJ", "NE", "NO", "LAR", "PHI", "PIT", "LAC", "SF", "SEA", "STL", "TB", "TEN", "WAS");
...
<p><a href="https://www.fantasyfootballnerd.com/fantasy-football-api" target="_blank">Click here to request an API key.</a></p>
...
<p>*Please note that your API key from Fantasy Football Nerd (FFN) gives you access to the same content that your FFN account would give you access to. Much of the content is free; however, if you need more complete or advanced content, please consider getting a commercial license.</p>
...
<p>For complete instructions on how to implement the FFN fantasy football plugin, be sure to visit our <a href="https://www.fantasyfootballnerd.com/wordpresss" target="_blank">WordPress How-To Guide</a>.</p>
</div>
<?php } ?>